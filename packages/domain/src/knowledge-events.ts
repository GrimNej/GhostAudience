...
export type KnowledgeEvent =
  | {
      ...
      readonly type: "FACT_ADDED";
      readonly fact: AudienceFact;
    }
  | {
      readonly operationId: string;
      readonly type: "FACT_SUPERSEDED";
      readonly factId: string;
      readonly supersededByFactId: string;
    }
  | {
      readonly operationId: string;
      readonly type: "ASSUMPTION_ADDED";
      readonly assumption: AudienceAssumption;
    }
  | {
      readonly operationId: string;
      readonly type:
...
export function applyKnowledgeEvents(
  ...
  for (const event of events) {
    switch (event.type) {
      case "FACT_ADDED":
        if (factMap.has(event.fact.id))
          throw new Error(`Fact ID collision: ${event.fact.id}.`);
        factMap.set(event.fact.id, event.fact);
        break;
      case "FACT_SUPERSEDED": {
        const existing = factMap.get(event.factId);
        if (existing === undefined || !factMap.has(event.supersededByFactId))
          throw new Error("Invalid fact supersession event.");
        factMap.set(event.factId, {
          ...existing,
          supersededByFactId: event.supersededByFactId,
        });
        break;
      }
      case "ASSUMPTION_ADDED":
        if (assumptionMap.has(event.assumption.id))
          throw new Error(`Assumption ID collision: ${event.assumption.id}.`);
      ...
      case "ASSUMPTION_EXPIRED": {
        ...
        if (existing === undefined)
          throw new Error(`Unknown assumption ${event.assumptionId}.`);
        const status =
          event.type === "ASSUMPTION_CONFIRMED"
            ? "confirmed"
            : event.type === "ASSUMPTION_REFUTED"
              ? "refuted"
              : "expired";
        assumptionMap.set(event.assumptionId, {
          ...existing,
          status,
