...
export function evaluateIntentLocally(
  ...
  const normalizedQuestions = state.questions.map((question) =>
  ...
  return [
    ...intent.requiredKnowledge.map((target) => ({
      ...
      status:
        target.targetOrdinal !== null && currentOrdinal < target.targetOrdinal
          ? ("not_yet_met" as const)
          : normalizedFacts.some((statement) =>
                statement.includes(target.statement.toLocaleLowerCase()),
              )
            ? ("met" as const)
            : ("not_yet_met" as const),
      explanation:
        "Compared only after neutral audience-state inference; this target never enters the model prompt.",
    })),
    ...intent.desiredQuestions.map((target) => ({
      targetId: target.id,
      kind: "desired_question" as const,
      status: normalizedQuestions.some((question) =>
        question.includes(target.question.toLocaleLowerCase()),
      )
        ? ("met" as const)
        : ("not_yet_met" as const),
    ...
    ...intent.forbiddenAssumptions.map((target) => ({
      ...
      status: normalizedAssumptions.some((assumption) =>
        assumption.includes(target.assumption.toLocaleLowerCase()),
      )
        ? ("violated" as const)
        : ("met" as const),
