...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= policy.maximumAttempts; attempt += 1) {
    ...
    try {
      return await operation(attempt, outerSignal);
    } catch (error: unknown) {
      lastError = error;
      ...
      const jitterMs = Math.random() * policy.jitterMs;
      const delayMs = explicitRetryMs ?? exponentialMs + jitterMs;

      await abortableDelay(delayMs, outerSignal);
    }
  }
